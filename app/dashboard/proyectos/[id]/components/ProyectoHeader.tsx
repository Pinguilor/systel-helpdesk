...
import { ChevronDown, User, Calendar, MapPin, Loader2, Lock } from 'lucide-react';
...
import { CierreProyectoModal } from './CierreProyectoModal';
...
export function ProyectoHeader({ proyecto, currentUserRol, currentUserId, responsableId }: ProyectoHeaderProps) {
    ...
    // 'completado' solo se alcanza mediante CierreProyectoModal, nunca desde el dropdown
    const ESTADOS_PERMITIDOS = (Object.keys(PROYECTO_ESTADO_CONFIG) as ProyectoEstado[]).filter(
        e => e !== proyecto.estado && e !== 'completado'
    );

    // Normalizar a lowercase para comparaciones independientes del caso recibido desde layout.tsx
    const rol          = (currentUserRol ?? '').toLowerCase();
    const isCompletado = proyecto.estado === 'completado';
    const canClose     = !isCompletado && (
        rol === 'admin' ||
        rol === 'coordinador' ||
        (!!currentUserId && currentUserId === responsableId)
    );
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                        ...
                        {/* Badge de estado + botón Cerrar Proyecto */}
                        <div className="flex items-center gap-2 shrink-0">
                            {/* Badge / dropdown de estado */}
                            <div className="relative">
                                {rol === 'tecnico' || isCompletado ? (
                            ...
                            {/* Botón Cerrar Proyecto — siempre visible para quien puede cerrar */}
                            {canClose && (
                                <button
                                    onClick={() => setIsCierreModalOpen(true)}
                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-500 hover:bg-rose-600 active:bg-rose-700 text-white rounded-xl text-xs font-black transition-all shadow-sm hover:shadow"
                                >
