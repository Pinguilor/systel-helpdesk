...
import {
    Wrench, Trash2, Loader2, AlertTriangle, CheckCircle2,
    X, Package, Hash, Plus, MinusCircle, PlusCircle,
} from 'lucide-react';
import {
    ajustarStockAction, darBajaSeriesAction,
    quitarGrupoBodegaAction, addStockToBodegaAction,
} from './actions';
...
function QuitarModal({ grupo, onClose }: { grupo: StockGroup; onClose: () => void }) {
    const router = useRouter();
    const [isPending, startTransition] = useTransition();
    const [alert, setAlert] = useState<{ type: 'error' | 'warning'; msg: string } | null>(null);
    const allIds = grupo.rows.map(r => r.id);

    function handleConfirm() {
        setAlert(null);
        startTransition(async () => {
            const result = await quitarGrupoBodegaAction(grupo.bodegaId, allIds);
            if (result.error) {
                setAlert({ type: 'error', msg: result.error });
            } else if ('softDeleted' in result && result.softDeleted) {
                setAlert({ type: 'warning', msg: 'El ítem tiene historial en tickets y no puede borrarse por completo. Su stock se ha vaciado y se ha ocultado del catálogo activo.' });
                setTimeout(() => { onClose(); router.refresh(); }, 2500);
            } else {
                onClose();
                router.refresh();
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            ...
            <div className="relative z-10 bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-sm p-6 flex flex-col gap-4"
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-sm font-black text-slate-800">Quitar de bodega</p>
                        <p className="text-xs text-slate-500 truncate max-w-[220px]">{grupo.modelo}</p>
                    </div>
                </div>
                <p className="text-sm text-slate-600">
                    Se eliminarán <span className="font-bold">{allIds.length} ítem(s)</span>
                    {grupo.es_serializado && ` (${allIds.length} serial(es))`} del inventario de esta bodega.
...
export function BodegaStockTable({ grupos }: { grupos: StockGroup[] }) {
    ...
    const [quitarGrupo, setQuitarGrupo] = useState<StockGroup | null>(null);
    ...
    return (
        <>
            ...
            {quitarGrupo && <QuitarModal grupo={quitarGrupo} onClose={() => setQuitarGrupo(null)} />}
        </>
    );
}
